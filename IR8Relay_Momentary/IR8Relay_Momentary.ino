/* Momentary Switch IR 8 Channel Relay Control
...
the appropiate relay turns ON an acts as a Momentary Switch. This means
that the relay will turn on and stay on while button is pressed and will
turn off only if released.
...
int Relay_i;
const int timeoutDelay = 150;  // milliseconds - I'm guessing here, this needs to be longer than the IR repeat interval
unsigned long IRcode;  // unsigned long max 4 bytes
...
void loop()
{
  if (My_Receiver.GetResults(&My_Decoder)) 
  {
    ...
    if (My_Decoder.decode_type == NEC) 
    {
      ...
      else if (state == 0 ) //This condition ensures that only one relay is turned on at a time
      {      
        if (IRcode == ONE)
        {
